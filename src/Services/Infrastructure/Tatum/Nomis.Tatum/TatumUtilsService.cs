...
namespace Nomis.Tatum
{
        ...
        ISingletonService
    {
        ...
        {
            var response = await _client.GetAsync($"/v3/tatum/rate/{currency.ToString()}?basePair={basePair.ToString()}");
            response.EnsureSuccessStatusCode();

            var data = await response.Content.ReadFromJsonAsync<TatumExchangeRate>();
            return data;
        }
